/**
 * landmarks_seed_data.ts
 *
 * 原本 hardcoded 在 HualienAddressDB.ts 的地標資料，抽出作為：
 *   1. 初次 seed_landmarks.ts 匯入 DB 的來源
 *   2. App 端離線 fallback 資料的產生來源（未來可考慮）
 *
 * 此檔僅在 seed 階段使用，正常運行時地標由 DB 載入。
 * 不要再手動新增地標到此檔 — 請透過 Admin Panel 新增到 DB。
 */
...
export const LANDMARKS_SEED: SeedLandmark[] = [
